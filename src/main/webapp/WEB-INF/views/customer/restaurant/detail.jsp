...
<script>
            ...
            <div id="item-header-top">
                <div id="item-header-left">
                    <div id="item-rn">
                        ...
                        <span class="item-rn-subtitle"><a>${r.genre}</a>,<a>${r.keyword}</a></span>
                    </div>
            ...
            <div id="item-header-bottom">
                <div id="item-header-left">
                    ...
                    <div id="item-stats">
                        ...
                        <div class="item-stats" dc-like-history-popover dc-like-history-modal mode="restaurant" id-key="OSEnXQWBYLcf" ng-if="profile.favorite.count>0">
    ...
    <div id="item-body">
        <div class="item-container" style="width:100%;">
            <div id="item-body-contents"><div class="item-information">
                    <div class="item-information-icon"><img src="https://dfzrjox9sv97l.cloudfront.net/dicons_20160930/img/profile/main/icon-profile-tag@2x.png"></div>
                    <div class="item-information-text"><a>${r.tip}</a></div>
        </div><div class="item-information">
                    <div class="item-information-icon"><img src="https://dfzrjox9sv97l.cloudfront.net/dicons_20160930/img/profile/main/icon-profile-address@2x.png"></div>
                    <div class="item-information-text">${r.address}</div>
                </div>
                <div class="item-information" id="item-tel">
                    <div class="item-information-icon"><img src="https://dfzrjox9sv97l.cloudfront.net/dicons_20160930/img/profile/main/icon-profile-call@2x.png"></div>
                    <div class="item-information-text">${r.phoneNumber}</div>
                </div>
            </div>
        </div>      
    </div><div style="margin:10px 0 30px 0;"><a href="JavaScript:setAdLog('{|u_id|:|852656|,|device|:|web|,|type|:|view|,|dcad_id|:|19|,|inven_id|:|63|,|inven_type|:|w_profile_middle_banner|,|image_id|:|503|,|v_rid|:|OSEnXQWBYLcf|,|page_type|:|profile|,|lat|:||,|lng|:||,|ip|:|10.253.32.54|}', 'https://box.diningcode.com/shop/item.php?it_id=1510644103');" target="_blank"><img src="https://d2u8ukm27m1oiz.cloudfront.net/banner/dcbox_ad_1511398484.jpg" width="960px" height="120" /></a></div><div style='width:960px; margin:0 auto;'>  <div class="rest-info-wrapper" style="float:left;"><div class="rest-time _flex_1">
        <div class="rest-info-icon"><img src="https://dfzrjox9sv97l.cloudfront.net/dicons_20160930/img/profile/main/icon-profile-time@2x.png"></div>
            ...
            <div class="rest-info-contents">
                    ...
                    <div  class="time">오전 11시 - 오후 9시</div></div>
                    </div>
                    <div class="rest-time-block">
                    <div class="rest-time-left">쉬는시간 매일</div>
                    <div class="rest-time-right">
                    <div  class="time">오후 3시 - 오후 5시</div></div>
                    </div> </div></div>
            <script type="text/javascript">
        ...
        <div class="rest-menu _flex_1">
            <div class="rest-info-icon"><img src="https://dfzrjox9sv97l.cloudfront.net/dicons_20160930/img/profile/main/icon-profile-menu@2x.png"></div>
            <div class="rest-info-title">메뉴 정보</div>
            <div class="rest-info-contents">
            <div class="rest-menu-block">
                <div class="rest-menu-left">1인당 평균가격</div>
                <div class="rest-menu-right"><div class="time">1만원대</div></div>
            </div>
            <div class="rest-menu-block">
                <div class="rest-menu-left">차돌박이 쌀국수</div>
                <div class="rest-menu-right"><div class="time">7,000원</div></div>
            </div>
            <div class="rest-menu-block">
                <div class="rest-menu-left">양지 쌀국수</div>
                <div class="rest-menu-right"><div class="time">7,500원</div></div>
            </div>
            <div class="rest-menu-block">
                <div class="rest-menu-left">차돌양지쌀국수</div>
                <div class="rest-menu-right"><div class="time">8,000원</div></div>
            </div></div><div class="more-extra" id="more-extra-menu">메뉴 더 보기<i class="fa fa-angle-down" aria-hidden="true"></i></div></div>
            <script type="text/javascript">
...
function setFavor(rv_id, write_id, favor) {
	var v_uid	= "6a225df186b8";
	if( !v_uid ) {
		alert("로그인이 필요한 기능입니다.");
		return;
	}
	else if( write_id == v_uid ) {
		alert("자신의 글은 공감 할 수 없습니다.");
		return;
	}

	favor		= ( $('#btn_favor_' + favor.toLowerCase() + '_' + rv_id).attr("check") == "act" ) ? "" : favor;
	var jData	= '{"mode" : "UPD_FAVOR", "rv_id" : "' + rv_id + '", "favor" : "' + favor + '", "u_id" : "852656"}';
	getAjax("//www.diningcode.com/ajax/review.php", "text", jData, setList);
}
...
function setList(r) {
	...
	if( arr.length == 1 ) {
		...
		var rv_id	= arr[2];
		var favor	= arr[3].toLowerCase();
		var active	= $('#btn_favor_' + favor + '_' + rv_id);
		var favor_y	= $('#btn_favor_y_' + rv_id);

		// 초기화
		favor_y.find("img").attr("src", favor_y.find("img").attr("src").replace("_act", "_on"));
		favor_y.attr("check", "on");

		// 버튼, 이미지 재설정
		if( arr[3] == "Y" ) {
			favor_y.find("img").attr("src", favor_y.find("img").attr("src").replace("_on", "_act"));
			favor_y.attr("check", "act");
		}

		// 수치 재설정
		favor_y.find("span").html(arr[0]);
	}
}

function getMoreContents(rv_id) {
	$('#div_review_' + rv_id).removeClass("ellipsis");
	$('#div_review_more_' + rv_id).hide();
}

var image_cnt	= 0;
var image_no	= 0;

function showImage(image, name, photo, date) {
	image_no	= 0;
	$('#lbl_name').html(name);
	$('#lbl_date').html(date);
    photo.replace('http://','https://');
	$('#div_photo').css("background-image", "url(" + photo + ")");

	$('#div_image').html("");
	image_cnt	= (image.length - 1);
	for( i=0; i<image.length; i++ ) {
		$('#div_image').append("<img src='" + image[i]['origin'] + "' style='max-width:640px; width:auto; max-height:460px; height:auto;' />");
	}
	getImage(0);
	$('#div_shadow').show();
	$('#div_viewer').show();
...
function getImage(n) {
	image_no   += n;
	if( image_cnt == 0 ) {
		$('#btn_left').addClass("image_paged");
		$('#btn_right').addClass("image_paged");
	}
	else {
		$('#btn_left').removeClass("image_paged");
		$('#btn_right').removeClass("image_paged");
	}
	...
	$('#div_image img').hide();
	$('#div_image img:eq(' + image_no + ')').show();
}

function getKeywordMore(mode) {
	$('#btn_keyword_down').toggle();
	$('#btn_keyword_up').toggle();

	if( mode ) {
		$('.keyword-list').height("100%");
	}
	else {
		$('.keyword-list').height("85");
	}
}

function getMore(device, page) {
	if( device == "web" ) {
		var u_id	= "D38cKoy78pCDWG1Soeec4Q==";
		if( !u_id ) {
			if( confirm("평가를 더 보시려면 로그인이 필요합니다.") ) {
				location.href	= "/user/?mode=login";
			}
			return;
		}
	}
	else if( device == "mobile" ) {
		if( confirm("평가 더보기는 다이닝코드 앱에서 가능합니다. 앱으로 이동하시겠습니까?") ) {
			clickInApp();
		}
		return;
	}

	var jData	= {"page" : page, "r_id" : "248469", "u_id" : "852656"};
	$.post('/Xeroad/review.php', jData, function(data) {
		$('#div_review_' + page).html(data);
		$('#div_loading').hide();
	});
}
</script> -->
...
<div style="padding:24px 0; border-bottom:#E0E0E0 solid 1px;">
	<div>
		...
		<div class="fl" style="padding:5px 0 0 12px;">
			...
			<div style="padding:8px 0;"><i class="fa fa-star" aria-hidden="true" style="width:20px; margin-top:-5px; font-size:18px; color:#2483ff;"></i><i class="fa fa-star" aria-hidden="true" style="width:20px; margin-top:-5px; font-size:18px; color:#2483ff;"></i></div>
			<div class="fs-15" style="width:888px; padding:8px 0; line-height:27px;">${c.content}</div>

			<div>	<!--// Image -->
				<div class="review_image">
					<div style='width:180px; height:125px; overflow:hidden;'><img src='https://s3-ap-northeast-1.amazonaws.com/dcreviewsresized/300_300_20171203094217183_photo1_557fcd08bcab.jpg' width='180' onClick='showImage([{"origin":"https:\/\/s3-ap-northeast-1.amazonaws.com\/dcreviewsresized\/20171203094217183_photo1_557fcd08bcab.jpg","thumb":"https:\/\/s3-ap-northeast-1.amazonaws.com\/dcreviewsresized\/300_300_20171203094217183_photo1_557fcd08bcab.jpg"},{"origin":"https:\/\/s3-ap-northeast-1.amazonaws.com\/dcreviewsresized\/20171203094216804_photo0_557fcd08bcab.jpg","thumb":"https:\/\/s3-ap-northeast-1.amazonaws.com\/dcreviewsresized\/300_300_20171203094216804_photo0_557fcd08bcab.jpg"}], "sue", "https://dfzrjox9sv97l.cloudfront.net/dicons_20160930/img/review/ic_profile_review_default.png", "1일 전");' style='cursor:pointer;' /></div>	<div class="review_ico">		<img src="https://dfzrjox9sv97l.cloudfront.net/dicons_20160930/img/review/icProfileReviewMenu.png" class="fl" />		<div class="review_count">2</div>		<div class="clear"></div>	</div>				</div>
				<div class="review_image">
					<div style='width:180px; height:125px; overflow:hidden;'><img src='https://s3-ap-northeast-1.amazonaws.com/dcreviewsresized/300_300_20171203094252679_menu0_557fcd08bcab.jpg' width='180' onClick='showImage([{"origin":"https:\/\/s3-ap-northeast-1.amazonaws.com\/dcreviewsresized\/20171203094252679_menu0_557fcd08bcab.jpg","thumb":"https:\/\/s3-ap-northeast-1.amazonaws.com\/dcreviewsresized\/300_300_20171203094252679_menu0_557fcd08bcab.jpg"}], "sue", "https://dfzrjox9sv97l.cloudfront.net/dicons_20160930/img/review/ic_profile_review_default.png", "1일 전");' style='cursor:pointer' /></div>	<div class="review_ico">		<img src="https://dfzrjox9sv97l.cloudfront.net/dicons_20160930/img/review/icProfileReviewStoreinfo.png" class="fl" /> 		<div class="review_count">1</div>		<div class="clear"></div>	</div>				</div>
				<div class="clear"></div>
			</div>	<!-- image // -->

			<div style="width:880px; padding-top:8px;" style="display:none;">
					<div class="keyword-text">점심식사</div>	<div class="keyword-text">저녁식사</div>	<div class="keyword-text">조용한</div>	<div class="keyword-text">아담한</div>	<div class="clear"></div>			</div>
		</div>
		<div class="clear"></div>
	</div>
...
<div id="seemore_review" style="margin:0;" onClick="$(this).remove(); $('#div_loading').show();">
	<a href="JavaScript:getMore('web', 2);">평가 더 보기  <i class="fa fa-angle-down" aria-hidden="true"></i></a>
</div>
<div id="div_loading" style="margin:5px 0; width:100%; text-align:center; display:none;">
...
<div id="div_review_2" style="margin:0;"></div>
                ...
                <div class="review-wrapper" style="margin:0; padding:0; height:0; overflow:hidden;"><div id="review-title">유저 평가</div><div class="review-list"><div id="review-area">
                <dc-review ng-repeat="item in profile.review" ng-model="item" index="$index"></dc-review>
    
                <div id="seemore_review" ng-if="profile.review_total-profile.review.length>0 && !profile.loading.review" ng-click="profile.load_more_review()">
